...
**核心问题**: 多 Agent 模式是否适合小说创作？三省六部架构是否存在问题？
...
3. **三省六部架构存在过度工程化问题**，不符合小说创作的核心需求
...
| **三省六部** | Arboris-Novel | 7 | 消息总线 + 权限矩阵 | ❌ 过度工程化 |
...
- ✅ **可组合**：工具可以自由组合
- ✅ **可测试**：每个工具独立测试
- ❌ **Agent 协作 + 消息传递**（三省六部的问题）

#### 4. **单一职责 > 多重职责**
- ✅ **每个 Agent 只做一件事**
- ✅ **职责清晰**：Architect / Writer / Editor
- ✅ **边界明确**：输入输出定义清晰
- ❌ **太子省/中书省/尚书省职责重叠**（三省六部的问题）

#### 5. **评审工具 > 协作伙伴**
- ✅ **Agent 作为评审器**：检查质量、连续性、风格
- ✅ **Agent 作为生成器**：生成内容、大纲、角色
- ❌ **Agent 作为协作伙伴**：消息传递、权限控制（三省六部的问题）
...
### 问题 1: 过度工程化
**现状**:
- 7 个 Agent：太子省、中书省、尚书省、兵部、户部、吏部、门下省
...
**现状**:
- 三省六部：7 个 Agent，消息传递，信息损失
- 传统管道：单 Agent，直接生成，无信息损失

**问题**:
- ❌ **三省六部的 7 个 Agent 引入 6 次消息传递**
- ❌ **每次消息传递都有信息损失**
- ❌ **最终生成质量 < 单 Agent**
...
2. 迁移现有逻辑：
   - 太子省 + 中书省 → PlannerAgent
   - 兵部 + 户部 → WriterAgent
   - 门下省 + 吏部 → ReviewerAgent
3. 移除消息总线：直接函数调用
4. 移除权限矩阵：无需权限控制
...
4. ❌ **三省六部过度工程化**：7 个 Agent + 消息总线 + 权限矩阵
