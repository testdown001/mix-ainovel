...
7. 不新增形容词、比喻、身体反应或环境抒情；最后两段不做风格化改写
...
2. 找到"一笔带过"但其实是关键时刻的地方，展开动作过程、阻力、选择和即时结果；感官只补理解动作所必需的一处
3. 对话中删掉"太配合"的回答——有时角色应该答非所问、沉默、或岔开话题
4. 至少制造 1 处"省略"——用一个动作或沉默代替本可以写出来的情绪/解释
5. 把连续超过100字的纯设定/纯解释段落打散成角色能感知的小细节
6. 当前 POV 的简短内心判断可以保留；只有重复总结或替非 POV 角色确认内心时，才改为可观察线索或当前 POV 的推断
7. 删除不提供新信息的形容词、副词和装饰性比喻；同一情绪节拍最多保留一个身体反应
8. 最后两段只保留具体动作、台词、发现或决定，不新增总结、未来预告和环境/命运隐喻
...
- 可以用不完整句、省略、短句独立成段等手法，但力度必须来自动作、选择或台词
- 优先精确名词和动词；一个句子只留一个主要意象，不堆形容词、比喻或身体反应
- 严格保持当前 POV，不确认其他角色的内心、幕后行动或未来结果
- 字数可以±20%浮动
