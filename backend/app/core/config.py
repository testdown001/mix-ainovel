...
class Settings(BaseSettings):
    ...
    generation_time_budget_sec: int = Field(
        ...
        description=(
            "单章生成关键路径软预算(秒，从生成总起点起算)。后处理链每步前检查剩余预算，"
            "不足以再跑一整步(约180s)即跳过剩余可选后处理(修订/一致性/人味化/优化/润色/扩写/"
            "压缩/六维审查/护栏 LLM 重写)、带当前最佳稿返回。保证后处理在预算内结束，避免冲破"
            "前端/网关 600s 硬超时致全盘失败。默认 540(留 60s 给收尾+回传)；<=0 关闭预算。"
            "注意：仅约束后处理；若上下文/正文生成本身(慢推理模型)已超时，需降 reasoning_effort"
        ),
    )
