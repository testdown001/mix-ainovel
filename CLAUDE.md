...
- **Client disconnect = generation really stops** (SSE): `_event_generator`'s `finally` cancels the producer task, so nothing continues server-side. `CancelledError` is not an `Exception` subclass, so the `except HTTPException/Exception` arms never saw it — until 2026-08-14 a disconnect refunded credits but left `chapter.status='generating'` **forever** (the frontend then spins + polls every 10s against a corpse, and users re-click generate = one wasted upstream run). Both sync entries now run `_mark_chapter_interrupted_safely` (own session + shield, log-only on failure) in the cancel path. Defence in depth for process death (container restart/OOM kill, where no handler runs at all): `utils/chapter_status.effective_chapter_status` presents a `generating` row older than 30 min as `failed` at **read time only** — no write, so a multi-replica deploy can't have replica A declare replica B's in-flight chapter dead.
