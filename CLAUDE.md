...
- `trace.log` — lightweight per-stage generation spans as JSON lines (logger `arboris.trace`); grep by `trace_id` to reconstruct one chapter generation's stage timeline (span / duration_ms / seq / status). Emitted via `app/utils/tracing.py` + `GenerationTelemetryService.mark_stage`. Since 2026-08-14 the standard post-processing chain also emits one `post_*` span per step (`post_combined_revision` / `post_consistency` / `post_humanization` / `post_optimizer` / `post_polish` / `post_enrichment` / `post_density_compression` / `post_six_dimension` / `post_six_dimension_rescore` / `post_auto_refine` / `post_guardrail_rewrite`) — before that the whole 6-10-call chain was a single opaque `stage_a_post_processing` span (measured at 39% of a chapter's wall time), and `standard_post_processing_service`'s `stage_timings_ms` dict was declared and returned but never written to, so attributing time to a step meant hand-aligning timestamps in `llm.log`. Steps skipped by the time budget deliberately record no timing.
...
- **Reasoning-model compat**: unified think-model detection (`_is_openai_reasoning_model` regex `^o[1-9]`, Claude thinking models); o-series drops `temperature`/`top_p`; retry loop self-heals on `max_completion_tokens` requirement, unsupported `response_format`/`stream_options`/`reasoning_effort` (each learned per `base_url|model` target in a process-level set), and auto-switches between chat/completions ↔ Responses endpoints.
- **Reasoning budget (latency lever, reworked 2026-08-14)**: `reasoning_effort` (minimal/low/medium/high) is per-channel + per-catalog-model configurable and is now sent on **any** openai/openai-responses model, not just o-series/gpt-5. The old model-name gate silently dropped the param for DeepSeek/Grok/GLM-class thinking models, so the admin setting had no effect on them — measured cost was ~55s per standard chapter (of 203s) burned on reasoning tokens in auxiliary calls, with one consistency check spending 8257 reasoning tokens to emit 2504 chars of JSON. Providers that reject the param get it dropped + latched after one retry (`_is_reasoning_effort_unsupported_error`, which deliberately does **not** treat an empty 400 body as its signal — that belongs to the `response_format` downgrade, and both reasoning branches must be evaluated *before* it). Auxiliary calls (any `response_format=json_object`: mission planning, consistency, summaries) are downgraded to SystemConfig `llm.aux_reasoning_effort` (default `low`, `off` disables, 60s process TTL cache); prose generation keeps the channel/catalog effort so paid-tier differentiation is unaffected. An explicit `reasoning_effort` argument from the call site always wins.
...
- **Channel testing**: `POST /api/admin/test-llm-channel` → `LLMService.test_channel` performs a real minimal call per channel, returns `{ok, configured, model, latency_ms, detail}`. Testable channels: `default/fallback/polish/search/grader/embedding/rerank` (`rerank` is not an LLM channel — `test_channel` forwards to `rerank_utils.test_rerank_connection`, which owns its own `configured` = `rerank.enabled` and still makes a real call when the switch is off so admins can validate before enabling). `configured=False` means the runtime won't use that channel at all, and **no test request is sent** — critically, `test_channel` must never substitute another channel's config for a disabled one. The enablement predicate for the optional channels (polish/search/grader) is `channel_explicitly_configured()` in `llm_service.py` — "any of the four channel keys (`api_key/base_url/model/api_format`) non-empty", since `api_key` may be left blank to inherit `llm.*`. That single predicate backs all three resolvers, `test_channel`, and the config audit; before it existed `test_channel` only looked at `api_key` and fell back to `llm.*`, so a genuinely disabled grader channel showed as 「✅ 可用」 in the admin health table while evidence grading was silently skipped (found live 2026-08-13, locked by `tests/test_channel_configured_state.py`). Unlike search/grader, polish **does** legitimately fall back to the default channel, so it counts as configured.
...
- **Credits (积分制) + Model Catalog**: on top of tiers, generation is metered in **credits**. Balance lives on `UserQuota` as **two pools** (2026-08-12): `credit_balance` 月度池 (`monthly_credit_grant`/`credit_carryover` — default reset-to-zero monthly, `credit_reset_at` rolling-reset anchor) + `credit_purchased` 永久池 (top-up purchases, never wiped by monthly reset — 「月赠清零、充值常驻」). Consumption spends monthly first then purchased; 402 checks the sum; refunds go back to the **monthly** pool (anti-laundering: deliberate failures must not convert expiring credits into permanent ones). Top-ups come from **加油包 credit packs**: catalog in SystemConfig `credits.packs` (JSON, defaults in `payment_service.DEFAULT_CREDIT_PACKS`), endpoints `GET /api/payment/credit-packs` + `POST /api/payment/create-credit-order`; pack orders are `PaymentOrder(plan_id=0, remark="credit_pack:{code}:{credits}")` and all three channels' callbacks route through `_activate_membership`, which detects the marker and calls `QuotaService.add_purchased_credits` (idempotent by `ref_key=order_no`) instead of membership upgrade. Also (2026-08-12): signup trial — first registration/OAuth/phone login grants 3-day creator + trial credits via `QuotaService.grant_signup_trial` (SystemConfig `trial.enabled/days/credits`, idempotent by `CreditLog(reason='trial', ref_key='signup:{user_id}')`); `Plan.monthly_credits` sets the per-plan grant written on activation (0 = tier default via `QuotaService._credit_grant_for_tier`). Every deduction/refund/grant is one `CreditLog` row (`credit_logs`), with a `(reason, ref_key)` unique constraint for **idempotency** and a `balance_after` snapshot. `ModelCatalog` (`model_catalog`) maps a frontend-selectable "模型" (章鱼1.0/2.0/3.0 = octopus_v1/v2/v3 placeholders) to a real LLM channel (5 keys `real_model/base_url/api_key_ref/api_format/reasoning_effort`, blank → fall back to `llm.*`) + `credit_price` + `min_tier`; admin CRUD + frontend tier-filtered display. Polish is a pure opt-in credit-billed add-on (2026-07-26): standard/premium presets no longer force-enable it — the user's checkbox (`flow_config.enable_polish`) both runs it and bills the surcharge (SystemConfig `credits.price.polish`, default 5); it is not tier-gated, the surcharge is charged even without a `model_code`, and a paid polish is never skipped by the generation time budget (paid-must-deliver). **Undelivered polish is refunded** (2026-08-13): `_run_polish` swallows every failure (channel error / empty response / non-prose output) and returns the unpolished text, so the chapter ships while the surcharge would otherwise stay charged. `polish_undelivered(result)` reads `review_summaries["polish"].applied` and `refund_polish_surcharge` returns just that surcharge —章节照常交付,只退没兑现的部分. It uses ref_key `{ref}:polish` (the `(reason, ref_key)` unique constraint allows only one refund per key), is mutually exclusive with the full refund, and caps cumulative refunds at the original charge. The polish unit price is written into the charge's `CreditLog.note` as `polish_unit=N` so an admin price change can't make the refund wrong. Wired into all four charging entries (sync / SSE / batch / async task); the async path can't see `review_summaries` (the worker returns a slim payload), so `_execute_chapter_generate` propagates a `polish_undelivered` flag the way it already does for `missing_scenes`. Related fix: when polish is merged into the optimizer, the summary used to report `applied: True` unconditionally — a failed optimizer returns its input text unchanged, so that was reporting undelivered work as delivered. **Billing wiring**: `generation_billing_service.py` (`compute_generation_cost`/`charge_generation` 先扣后跑, 402 on insufficient / `refund_generation` by `ref_key=task_id`, idempotent) and `QuotaService.consume_credits`/`refund_credits`/`has_credits`/`check_and_reset_credit`/`list_credit_logs`. Model tier gate: `core/feature_gating.ensure_model_allowed(session, model_code, effective_tier)` (403 if model `min_tier` > user tier or model inactive). `model_code` → real channel via `LLMService._resolve_config_by_model_code` (returns a `config_override` consumed by `single_version_generation_service`/`scene_generation_service`). **Charging happens on the async production task path only** (`task_worker.py`: gate + charge before dispatch, shielded refund on failure/cancel); the sync/SSE writer path passes `flow_config.model_code` through for **channel selection only** and does not charge. **Backward-compatible**: absent/unknown/inactive `model_code` → cost 0 → no charge, no behavior change. Endpoints: `/api/model-catalog/available` (tier-filtered models + credit balance + polish price), `/api/model-catalog/*` (admin CRUD), `/api/quota/me/credit-logs` (ledger). Frontend: `components/writing-desk/WDModelPicker.vue` (model + polish toggle + balance), `components/CreditLedger.vue` (流水明细), `components/admin/ModelCatalogConfig.vue`, `api/credits.ts` + `api/model_catalog.ts`.
- **Quality-loop switches (旗舰档质量回路)**: four opt-in loops — `outline_revision` (rolling outline revision), `volume_retrospective` (volume retrospective + re-plan), `character_significance` (人物意义层: belief shift / cost / relational meaning / unspoken, injected as *subtext only* — the prompt forbids stating it directly), `two_pass_draft` (lightly-constrained draft → constrained rewrite). Config lives in **SystemConfig `quality_loop.*`** (admin panel 「质量回路」, `QualityLoopConfig.vue`); `*_ENABLED` env vars are seeds only. All default off and are premium-preset-gated. The first three are async followups (degrade silently, never block generation); `two_pass_draft` sits **inside** the generation path and roughly doubles per-chapter cost.
- **Admin panel realities**: usage stats are real (every LLM call increments `UsageMetric` + `ApiUsageLog`); LLM channel test makes real calls; system configs CRUD at `/api/admin/system-configs`. Frontend admin components: `LoginAuthConfig` / `PaymentChannels` / `PaymentRecords` / `MembershipPlans` / `ApiManagement` / `ModelCatalogConfig` / `LLMDiagnostics` (「通道诊断」— per-channel LLM health + real-call telemetry + 配置体检: `GET /api/admin/llm-config-audit` → `services/llm_channel_audit.py`, read-only audit that catches what live probes can't — 假冗余 (fallback pointing at the default's upstream: error if identical target, warn if same base_url; blank fallback fields compare post-inheritance) and 静默失效 (no dedicated embedding config + non-OpenAI base_url = RAG silently dead; search/grader unconfigured = silent skips; rerank enabled without api_url) and 模型目录错配 (added 2026-08-14 after a live incident: a catalog row with a blank `base_url` inherits `llm.base_url`, so a `real_model` from a different vendor family — catalog `gpt-5.4` on an `api.deepseek.com` default channel — is rejected upstream and the **fallback channel silently ghost-writes** the chapter; all three price tiers then produce output from one and the same fallback model while every live channel probe stays green. `_model_family` prefix comparison flags it as an error at config-read time; a separate warn fires when two active rows resolve to the same `(base_url, model)`, i.e. a fake price ladder)) / `FeatureGatingConfig` / `Statistics` under `frontend/src/components/admin/`. `UserManagement` also has an 「即将到期」 view (`GET /api/admin/users/expiring?days=7|15|30`, capped 1–90) — expiry is the only automatic churn point (all three channels are one-shot payments, expiry silently drops the user to `free`), and the whole-table user list can't surface it. `has_paid_order` splits 试用未转化 from 付费续费 (different outreach), `reminded` mirrors `expiry_reminded_for` so manual outreach doesn't double up on the automatic email.
...
- `composables/` — `useAsyncGeneration.ts`, gateway WebSocket progress `useWebSocket.ts`, `useAlert.ts`, `useGenerationProgress.ts` (the single generation-progress state machine: every source calls `applyStage`, percent is monotonic, and channel degradation — gateway down → direct SSE, WS dropped → 2s polling — is state, surfaced to the user rather than swallowed; `ChapterGenerating.vue` renders it and no longer keeps its own keyword→percent table)
...
3. Optional **reranker**: hybrid path fetches 2×top_k then reranks via a Jina-compatible API (`utils/rerank_utils.py`). Config is **SystemConfig `rerank.enabled/api_url/api_key/model`** (admin panel 「接口管理 → 重排序模型」, with a real-call 「测试连接」 button); `RAG_RERANKER_*` env vars are seeds only. **`rerank.api_url` is used verbatim** (trailing slash stripped only) — never path-append, vendor rerank paths differ (`/v1/rerank`, `/v1/rerank/multimodal`, …). Only the legacy fallback (url/key unset → borrow `embedding.base_url`/`embedding.api_key`) derives an endpoint by appending `/rerank`. A per-URL failure latch (3 consecutive failures) extinguishes rerank **per process** so a dead endpoint can't burn a round-trip on every retrieval — a successful call or a successful admin test clears it (only in that process).
...
- **Progress vocabulary (one table per side, keyed by stage key)**: the pipeline's `emit_stage` keys (`starting` / `build_generation_prompt` / `generate_versions` / `post_*` / `persist_versions` / `completed` / `agent:*`) are the contract. Since 2026-08-14 `standard_post_processing_service` announces **each** post step at its start (`_begin` → `emit_stage`, labels in `POST_STAGE_LABELS`) — before that the chain (~40% of a chapter's wall time) was completely silent and the UI sat on 「多版本生成中」 for over a minute, which reads as a hang. Percent mapping lives in exactly two places, both keyed by stage key rather than by matching Chinese words inside the message prose: `task_worker._STAGE_PROGRESS` (async path, caps at 79; a keyword table remains as fallback for `agent:*`) and `frontend/src/utils/generationStages.ts` (labels + percent for every source). Reword a stage message and nothing breaks; add a stage and you add one row on each side.
- **Time-budget skips are disclosed** (2026-08-14): each optional post step needs `_PER_STEP_RESERVE_SEC` (180s) of remaining budget or it is skipped, and the result carries `review_summaries.time_budget.skipped`. That was written but never read — on a slow upstream the whole chain silently vanishes (measured live: a standard chapter took 380s — 120s mission + 259s drafting — and *every* post step was skipped, so a paying standard-tier user got an unreviewed first draft with no indication). `utils/budgetSkip.ts` reads it from both payload shapes (`task_worker` lifts it into the async payload as `skipped_for_budget`, same trick as `missing_scenes`/`polish_undelivered`) and `WritingDesk` says which steps were skipped.
- **SSE must bypass the Go gateway** (`deploy/nginx.conf`, exact-match location, 2026-08-14): the gateway's reverse proxy is Fiber `proxy.Do()` on fasthttp, which reads the entire response body before returning — it cannot stream. Measured: direct to app, `started`/`stage` events arrive in 0.3s; through the gateway, 25s of complete silence. Every SSE feature (streaming draft, stage progress) was dead in production until this. Bypassing costs only gateway-side rate limiting; the endpoint still verifies JWT, tier-gates and charges credits, and FastAPI has its own rate-limit middleware.
- **Client disconnect does NOT stop generation** (SSE, measured on the prod stack 2026-08-14 by killing a `curl` mid-stream): nginx/uvicorn don't propagate the client abort into the response generator, so the producer runs to completion — versions land, status reaches `waiting_for_confirm`, async followups (stage-B, foreshadowing) still fire. A disconnect loses **the push, not the chapter**, which is why the frontend must reconcile against chapter state instead of guessing (`WritingDesk.reconcileInterruptedChapter`), and why `api/novel.ts` throws a distinct `StreamInterruptedError` (`utils/streamInterruption.ts`) instead of the old「生成章节失败」— that message made users think their credits burned for nothing and re-click generate, which *is* a wasted upstream run.
- **If cancellation does reach the producer** (`_event_generator`'s `finally` cancels it; happens on direct uvicorn connections and would happen if a proxy started forwarding aborts): `CancelledError` is not an `Exception` subclass, so the `except HTTPException/Exception` arms never saw it — until 2026-08-14 that path refunded credits but left `chapter.status='generating'` **forever**. Both sync entries now also run `_mark_chapter_interrupted_safely` there. Awaiting compensation inside `except CancelledError` is safe (verified: a single delivered `cancel()` lets the handler's awaits complete, so the session outlives the commit) — but it stays wrapped in shield + log-only failure.
- **Ghost `generating` rows** from process death (container restart mid-generation, OOM kill — no handler runs at all; a real 18-day-old ghost was found on the test server): `utils/chapter_status.effective_chapter_status` presents a `generating` row older than 30 min as `failed` at **read time only** — no write, so a multi-replica deploy can't have replica A declare replica B's in-flight chapter dead. Both chapter-read paths (`novel_service._build_chapter_schema`, `writer.py`'s chapter fetch) go through it.
- **Async path (production)**: Go Dispatcher → `POST /api/internal/tasks/execute` (`task_worker.py`, `X-Internal-Secret` callbacks) → same `HybridExecutor`. The same tier gate runs here before dispatch; a too-high preset fails the task with the 403 detail and `permanent: true`, which the Go dispatcher honors by skipping retries. Credits are metered here via `ensure_model_allowed` + `charge_generation` (先扣后跑) before execution, with a shielded `refund_generation(ref_key=task_id)` on failure/cancel (see Credits + Model Catalog above). The three sync/SSE entries in `writer.py` run the **same** gate + charge + idempotent-refund set (added 2026-08-12 — before that, a gateway outage made the frontend fall back to SSE and paid model channels became free).
