  ...
  <aside class="chapter-rail">
    <div class="rail-head">
      ...
      <div class="rail-actions">
        ...
        <button
          type="button"
          :title="outlineTaskActive ? '章纲生成任务正在运行' : '生成或调整大纲'"
          :disabled="outlineTaskActive"
    ...
    <div ref="listEl" class="rail-list">
      ...
      >
        ...
        <span class="chapter-index" :class="statusClass(outline.chapter_number)">
          <svg
            v-if="statusOf(outline.chapter_number) === 'done'"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            stroke-width="2.2"
          ...
          <svg
            v-else-if="statusOf(outline.chapter_number) === 'working'"
            class="spin"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
    ...
    <div class="rail-foot">
      <WDOutlineTaskCard
        v-if="outlineTask"
        class="outline-task-in-rail"
        :task="outlineTask"
        @cancel="emit('cancelOutlineTask')"
        @retry="emit('retryOutlineTask')"
        @dismiss="emit('dismissOutlineTask')"
      />
...
const outlineTaskActive = computed(() =>
  Boolean(
...
.chapter-row.active .row-accent {
  background: #ffe500;
  box-shadow: 0 0 16px rgba(255, 229, 0, 0.24);
}
.chapter-index {
  display: grid;
  width: 36px;
...
.chapter-index.is-done {
  border-color: rgba(48, 211, 124, 0.25);
  color: #36d885;
  background: rgba(25, 126, 75, 0.15);
}
.chapter-index.is-working {
