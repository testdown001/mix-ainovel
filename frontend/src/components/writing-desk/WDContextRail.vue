  ...
  <aside class="context-rail">
    ...
    <div class="context-scroll">
      <details open class="context-section">
        <summary>
          <span><i class="section-dot character"></i>人物</span><b>{{ characters.length }}</b>
        </summary>
      ...
      <details open class="context-section">
        <summary>
          <span><i class="section-dot setting"></i>设定</span><b>{{ settings.length }}</b>
        </summary>
        <div class="context-items">
          <button
            ...
            :key="setting.key"
            type="button"
            class="context-item setting-item"
          >
            ...
            <span class="item-copy"
              ...
              ><small>{{ setting.summary }}</small></span
            >
            <span class="selected-mark">✓</span>
          </button>
      ...
      <details open class="context-section">
        <summary>
          <span><i class="section-dot clue"></i>伏笔</span><b>{{ foreshadowings.length }}</b>
        </summary>
        <div class="context-items">
          <button
            v-for="item in foreshadowings"
            :key="item.name || item.description"
            type="button"
            class="context-item clue-item"
          >
            <span class="clue-line"></span>
            <span class="item-copy"
              ><strong>{{ item.name || '未命名伏笔' }}</strong
              ><small>{{ foreshadowingMeta(item) }}</small></span
            >
            <span class="selected-mark">✓</span>
          </button>
      ...
      <details open class="context-section memory-section">
        <summary>
          <span><i class="section-dot memory"></i>创作记忆</span>
          <span class="summary-tools">
            <b>{{ activeMemories.length }}</b>
            <button
              type="button"
              class="refresh-memory"
              title="刷新创作记忆"
              :disabled="memoryLoading"
              @click.stop="loadCreativeMemories"
            >
              ↻
            </button>
          </span>
        ...
        <div class="context-items">
          <p v-if="!activeMemories.length && !candidateMemories.length" class="empty-copy">
            暂无规则。确认作者偏好后，会在这里留下可追溯依据。
          </p>
          <div
            v-for="item in activeMemories"
            :key="`active-${item.id}`"
            class="memory-item active-memory"
          >
            <div class="memory-item-head">
              <span class="memory-scope">{{ memoryScopeLabel(item) }}</span>
              <span v-if="usedMemoryIds.has(item.id)" class="memory-used">本章已用</span>
            </div>
            <strong>{{ item.title }}</strong>
            <small>{{ item.content }}</small>
          </div>
          <div
            v-for="item in candidateMemories"
            :key="`candidate-${item.id}`"
            class="memory-item candidate-memory"
          >
            <div class="memory-item-head">
              <span class="memory-scope">待确认 · {{ memoryCategoryLabel(item.category) }}</span>
            </div>
            <strong>{{ item.title }}</strong>
            <small>{{ item.content }}</small>
            <div class="memory-actions">
              <button type="button" @click="confirmMemory(item.id)">确认规则</button>
              <button type="button" class="muted-action" @click="rejectMemory(item.id)">
                忽略
              </button>
            </div>
          </div>
...
const memoryItems = ref<CreativeMemoryItem[]>([])
const latestMemoryIds = ref<number[]>([])
const memoryLoading = ref(false)
const activeMemories = computed(() => memoryItems.value.filter((item) => item.status === 'active'))
const candidateMemories = computed(() =>
  memoryItems.value.filter((item) => item.status === 'candidate'),
)
const usedMemoryIds = computed(() => new Set(latestMemoryIds.value))

async function loadCreativeMemories() {
  if (!props.project?.id) return
  memoryLoading.value = true
  try {
    const result = await creativeMemoryApi.list(props.project.id, props.selectedChapterNumber)
    memoryItems.value = result.items || []
    latestMemoryIds.value = result.latest_receipt?.memory_ids || []
  } catch {
    // 创作记忆是辅助信息，加载失败不应影响正文编辑。
  } finally {
...
async function confirmMemory(memoryId: number) {
  await creativeMemoryApi.update(props.project.id, memoryId, { status: 'active' })
  await loadCreativeMemories()
}

async function rejectMemory(memoryId: number) {
  await creativeMemoryApi.update(props.project.id, memoryId, { status: 'rejected' })
  await loadCreativeMemories()
}

function memoryScopeLabel(item: CreativeMemoryItem): string {
  const labels = {
    author: '作者习惯',
    novel: '本书规则',
    volume: `第${item.volume_number || '?'}卷`,
    chapter: `第${item.chapter_number || '?'}章`,
  }
  return labels[item.scope]
}

function memoryCategoryLabel(category: CreativeMemoryItem['category']): string {
  const labels: Record<CreativeMemoryItem['category'], string> = {
    style: '风格',
    viewpoint: '视角',
    rhetoric: '修辞',
    dialogue: '对话',
...
watch(() => [props.project?.id, props.selectedChapterNumber], loadCreativeMemories)
onMounted(loadCreativeMemories)
...
function foreshadowingMeta(item: BlueprintForeshadowing): string {
  if (item.target_chapter)
    return `第 ${item.target_chapter} 章前回收 · ${item.tier || item.type || '普通'}`
...
.context-scroll {
  ...
  scrollbar-color: #33352f transparent;
}
.context-section {
...
.section-dot.memory {
  background: #d7c423;
  box-shadow: 0 0 10px rgba(255, 229, 0, 0.22);
}
.summary-tools {
  display: flex;
  align-items: center;
...
.candidate-memory {
  border-color: #514a1a;
  background: rgba(255, 229, 0, 0.035);
}
.memory-actions {
  display: flex;
  gap: 5px;
...
.memory-actions button {
  padding: 4px 7px;
  border: 1px solid #7a6f16;
  border-radius: 5px;
  color: #f2d900;
  font-size: 8px;
  background: rgba(255, 229, 0, 0.08);
}
.memory-actions .muted-action {
  border-color: #34352d;
  color: #81847c;
  background: #191a17;
}
...
.clue-line {
  ...
  height: 2px;
  border-radius: 9px;
  background: linear-gradient(90deg, #805d8c, #c2a3cc);
}
.empty-copy {
  margin: 3px 7px 8px;
  color: #5f625a;
...
.ai-card {
  margin-top: 15px;
  padding: 15px;
  border: 1px solid #3a3826;
  border-radius: 12px;
  background: linear-gradient(145deg, rgba(255, 229, 0, 0.045), rgba(21, 22, 18, 0.8));
}
.ai-card-head {
  display: flex;
  align-items: center;
...
.ask-button {
  ...
  margin-top: 11px;
  padding: 0 11px;
  border: 1px solid #625c24;
  border-radius: 8px;
  color: #dfd01d;
  font-size: 9px;
  font-weight: 700;
  background: rgba(255, 229, 0, 0.055);
}
.ask-button:hover {
  border-color: #92861d;
