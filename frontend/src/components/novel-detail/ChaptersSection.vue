...
<template>
  <div class="flex flex-col h-full min-h-0 overflow-hidden relative">
    <div class="flex flex-row flex-1 h-full lg:min-h-0 overflow-hidden">
      ...
      <!-- 章节列表侧边栏 -->
      <aside
        class="fixed lg:static inset-y-0 left-0 z-50 w-72 lg:w-72 bg-bg-surface lg:bg-bg-elevated border-r border-border flex flex-col h-full min-h-0 max-h-full overflow-hidden transition-transform duration-300 lg:translate-x-0 lg:shadow-none"
        :class="showChapterList ? 'translate-x-0' : '-translate-x-full'"
      >
        <div class="px-5 py-4 border-b border-border flex items-center justify-between">
          <h3 class="text-base font-semibold text-text-primary">章节</h3>
          <span class="text-xs text-text-muted">{{ chapters.length }} 篇</span>
        </div>
        <ul class="flex-1 h-full overflow-y-auto divide-y divide-border overscroll-contain">
          <li v-for="(chapter, index) in chapters" :key="chapter.chapter_number">
            <button
              ...
              :class="selectedChapter?.chapter_number === chapter.chapter_number ? 'bg-primary-muted text-primary font-semibold' : 'hover:bg-[rgba(255,255,255,0.05)] text-text-secondary'"
              @click="selectChapter(chapter.chapter_number)"
            >
              <div class="flex items-center justify-between gap-3">
                <div class="flex items-center gap-3 min-w-0">
                  <span class="inline-flex items-center justify-center w-6 h-6 text-xs font-semibold text-text-muted bg-bg-elevated rounded-full">
                    {{ index + 1 }}
                  </span>
                  <span class="truncate">{{ chapter.title || `第${chapter.chapter_number}章` }}</span>
                </div>
                <span v-if="chapterCache.has(chapter.chapter_number)" class="text-xs text-text-muted">
                  {{ calculateWordCount(chapterCache.get(chapter.chapter_number)?.content) }} 字
                </span>
                <span v-else class="text-xs text-text-muted">-</span>
              </div>
              <p v-if="chapter.summary" class="mt-1 text-xs text-text-muted truncate">
                {{ chapter.summary }}
              </p>
            </button>
          </li>
      ...
      <section class="flex-1 flex flex-col bg-bg-surface h-full min-h-0 max-h-full overflow-hidden relative">
        <!-- 移动端浮动按钮 -->
        <button
          v-if="!showChapterList"
          @click="showChapterList = true"
          class="lg:hidden fixed bottom-6 left-6 z-30 w-14 h-14 bg-primary text-on-primary rounded-full flex items-center justify-center hover:bg-primary-hover transition-colors"
        >
        ...
        <div v-if="isLoading" class="h-full flex items-center justify-center">
          <div class="text-center">
            <div class="w-10 h-10 border-4 border-primary-muted border-t-primary rounded-full animate-spin mx-auto mb-3"></div>
        ...
        <div v-else-if="error" class="h-full flex items-center justify-center">
          <div class="text-center">
            <div class="w-12 h-12 bg-error-muted rounded-full flex items-center justify-center mx-auto mb-3">
        ...
        <template v-else-if="selectedChapter">
          ...
          <header class="px-6 py-4 border-b border-border bg-bg-elevated">
            <div class="flex items-start justify-between gap-4 mb-3">
              <div class="flex-1">
                <h4 class="text-xl font-bold text-text-primary">{{ selectedChapter.title || `第${selectedChapter.chapter_number}章` }}</h4>
                <div class="flex items-center gap-3 mt-1.5">
                  <span class="text-sm text-text-muted">第 {{ selectedChapter.chapter_number }} 章</span>
                  <span class="text-sm text-text-muted">·</span>
                  <span class="text-sm text-text-muted">{{ calculateWordCount(selectedChapter.content) }} 字</span>
                </div>
              </div>
              <div class="flex items-center gap-2">
                <button
                  ...
                  :class="selectedChapter?.content ? 'border-primary/20 text-primary hover:bg-primary-muted' : 'border-border text-text-muted cursor-not-allowed'"
            ...
            <div class="flex gap-1">
              ...
              >
                ...
                <span v-if="tab.badge && getTabBadgeCount(tab.key)"
                  ...
                  :class="activeTab === tab.key ? 'bg-primary-muted text-primary' : 'bg-bg-highlight text-text-secondary'">
            ...
            <div v-show="activeTab === 'content'" class="px-2 py-3">
              <div class="max-w-full space-y-4">
                ...
                <div v-if="selectedChapter.summary || selectedChapter.real_summary" class="grid gap-4">
                  <div v-if="selectedChapter.summary" class="bg-primary-muted border border-primary/20 rounded-xl p-4">
                    <h5 class="text-xs font-semibold text-primary mb-2 flex items-center gap-1.5">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                      </svg>
                      计划大纲
                    </h5>
                    <p class="text-sm text-primary leading-relaxed">{{ selectedChapter.summary }}</p>
                  </div>
                  <div v-if="selectedChapter.real_summary" class="bg-success-muted border border-success/20 rounded-xl p-4">
                    <h5 class="text-xs font-semibold text-success mb-2 flex items-center gap-1.5">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
                      </svg>
                      实际内容概要
                    </h5>
                    <div class="prose prose-sm max-w-none text-success" v-html="renderMarkdown(selectedChapter.real_summary)"></div>
                  </div>
                </div>

                <!-- Main Content -->
                <div class="prose prose-slate max-w-none p-4 sm:p-6 rounded-xl bg-[var(--paper-card)]">
                  <div class="text-base text-text-primary leading-8 whitespace-pre-wrap font-serif">
              ...
              <div class="max-w-full">
                <div v-if="selectedChapter.versions && selectedChapter.versions.length > 0" class="space-y-4">
                  <div v-for="(version, index) in selectedChapter.versions" :key="index"
                    class="border border-border rounded-xl p-5 hover:border-[rgba(255,255,255,0.15)] transition-all duration-200 group cursor-pointer"
                    @click="openVersionModal(version, index)">
                    <div class="flex items-center justify-between mb-3">
                      <h5 class="text-sm font-semibold text-text-primary flex items-center gap-2">
                        <span class="w-6 h-6 bg-primary-muted text-primary rounded-full flex items-center justify-center text-xs font-bold">
                          {{ index + 1 }}
                        </span>
                        版本 {{ index + 1 }}
                      </h5>
                      <div class="flex items-center gap-3">
                        <span class="text-xs text-text-muted">{{ calculateWordCount(version) }} 字</span>
                        <span class="text-xs font-medium text-primary opacity-0 group-hover:opacity-100 transition-opacity">
                          点击查看全文 →
                        </span>
                      </div>
                    </div>
                    <div class="text-sm text-text-secondary leading-7 whitespace-pre-wrap line-clamp-4">
                      {{ version }}
                    </div>
                  </div>
                </div>
            ...
            <div v-show="activeTab === 'evaluation'" class="px-2 py-3">
              <div class="max-w-full">
                <div v-if="evaluationData" class="space-y-4">
                  <!-- 最佳选择 -->
                  <div v-if="evaluationData.best_choice" class="bg-primary-muted border border-primary/20 rounded-xl p-4">
                    <div class="flex items-start gap-4">
                      ...
                      <div class="flex-1">
                        <h5 class="text-lg font-bold text-primary mb-2">最佳版本选择</h5>
                        <div class="flex items-center gap-2 mb-3">
                          <span class="px-3 py-1 bg-primary text-on-primary text-sm font-bold rounded-full">
                            版本 {{ evaluationData.best_choice }}
                          </span>
                        </div>
                        <p v-if="evaluationData.reason_for_choice" class="text-sm text-primary leading-relaxed">
                  ...
                  <div v-if="evaluationData.evaluation" class="space-y-4">
                    <div v-for="(versionEval, versionKey) in evaluationData.evaluation" :key="versionKey"
                      ...
                      :class="isSelectedVersion(versionKey, evaluationData.best_choice) ? 'ring-2 ring-primary/30' : ''">
                      <!-- 版本标题 -->
                      <div class="px-5 py-3 bg-bg-elevated border-b border-border flex items-center justify-between">
                        <h6 class="font-bold text-text-primary flex items-center gap-2">
                          <span class="w-6 h-6 bg-bg-base text-white rounded-full flex items-center justify-center text-xs">
                            {{ getVersionNumber(versionKey) }}
                          </span>
                          {{ getVersionLabel(versionKey) }}
                        </h6>
                        <span v-if="isSelectedVersion(versionKey, evaluationData.best_choice)"
                          class="px-2.5 py-1 bg-primary-muted text-primary text-xs font-semibold rounded-full">
                          最佳
                        </span>
                      </div>
                      ...
                      <div class="p-4 space-y-3">
                        ...
                        <div v-if="versionEval.pros && versionEval.pros.length > 0"
                          class="bg-success-muted border border-success/20 rounded-lg p-3">
                          <h6 class="text-xs font-bold text-success mb-2 flex items-center gap-1.5">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <ul class="space-y-1.5">
                            <li v-for="(item, idx) in versionEval.pros" :key="idx"
                              class="flex items-start gap-2 text-xs text-success leading-relaxed">
                              <span class="w-1 h-1 bg-success rounded-full mt-1.5 flex-shrink-0"></span>
                              <span>{{ item }}</span>
                            </li>
                          </ul>
                        </div>
                        ...
                        <div v-if="versionEval.cons && versionEval.cons.length > 0"
                          class="bg-error-muted border border-error/20 rounded-lg p-3">
                          <h6 class="text-xs font-bold text-error mb-2 flex items-center gap-1.5">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <ul class="space-y-1.5">
                            <li v-for="(item, idx) in versionEval.cons" :key="idx"
                              class="flex items-start gap-2 text-xs text-error leading-relaxed">
                              <span class="w-1 h-1 bg-error rounded-full mt-1.5 flex-shrink-0"></span>
                              <span>{{ item }}</span>
                            </li>
                          </ul>
                        </div>
                        ...
                        <div v-if="versionEval.overall_review"
                          class="bg-primary-muted border border-primary/20 rounded-lg p-3">
                          <h6 class="text-xs font-bold text-primary mb-2">总体评价</h6>
                          <p class="text-xs text-primary leading-relaxed">{{ versionEval.overall_review }}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div v-else-if="evaluationData.decision || evaluationData.feedback" class="space-y-4">
                    <!-- 评审决策 -->
                    <div v-if="evaluationData.decision" class="bg-primary-muted border border-primary/20 rounded-xl p-4">
                      <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                          <h5 class="text-sm font-bold text-primary">评审决策</h5>
                          <p class="text-xs text-primary">{{ evaluationData.decision }}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div v-if="evaluationData.scores" class="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <div v-for="(score, key) in evaluationData.scores" :key="key"
                        class="bg-bg-surface border border-border rounded-xl p-4 transition-shadow">
                        <div class="flex items-center justify-between mb-2">
                          <span class="text-xs font-medium text-text-secondary">{{ getScoreLabel(key) }}</span>
                    ...
                    <div v-if="evaluationData.feedback"
                      class="bg-bg-elevated border border-border rounded-xl p-4">
                      <h5 class="text-sm font-bold text-text-primary mb-3">详细反馈</h5>
                      <p class="text-sm text-text-secondary leading-relaxed whitespace-pre-wrap">{{ evaluationData.feedback }}</p>
                    </div>
                  </div>
                </div>

                <div v-else class="text-center py-12">
                  <div class="w-16 h-16 bg-bg-elevated rounded-full flex items-center justify-center mx-auto mb-3">
                    <svg class="w-8 h-8 text-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                  </div>
                  <p class="text-text-muted">暂无评审意见</p>
                </div>
              </div>
            </div>
          </article>
        ...
        <div v-else class="h-full flex items-center justify-center text-text-muted">
          <div class="text-center">
            <svg class="w-16 h-16 mx-auto mb-3 text-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    >
      <div v-if="versionModal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
        ...
        <div class="bg-bg-surface rounded-2xl border border-border max-w-4xl w-full max-h-[85vh] overflow-hidden"
          @click.stop>
          <!-- Modal Header -->
          <div class="flex items-center justify-between px-6 py-4 border-b border-border bg-bg-elevated">
            <div class="flex items-center gap-3">
              <span class="w-8 h-8 bg-primary text-on-primary rounded-full flex items-center justify-center text-sm font-bold">
                {{ versionModal.index + 1 }}
              </span>
              <div>
                <h3 class="text-lg font-bold text-text-primary">版本 {{ versionModal.index + 1 }}</h3>
                <p class="text-xs text-text-muted">{{ calculateWordCount(versionModal.content) }} 字</p>
              </div>
            </div>
            <button @click="closeVersionModal"
              class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[rgba(255,255,255,0.05)] transition-colors">
