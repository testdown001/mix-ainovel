  ...
  <div class="space-y-6">
    <div class="bg-bg-surface rounded-2xl border border-border p-6">
      <div class="flex items-start justify-between gap-4 mb-4">
        <h3 class="text-lg font-semibold text-text-primary">核心规则</h3>
        <button
          v-if="editable"
          type="button"
      ...
      <p class="text-text-secondary leading-7 whitespace-pre-line">{{ worldSetting.core_rules || '暂无' }}</p>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="bg-bg-surface rounded-2xl border border-border p-6">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center text-text-primary font-semibold">
            <svg class="mr-2 text-primary" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 22V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v18"/><path d="M6 18H4a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h2v7Z"/><path d="M18 18h2a2 2 0 0 0 2-2v-3a2 2 0 0 0-2-2h-2v7Z"/></svg>
            <span>关键地点</span>
          </div>
          <button
            v-if="editable"
        ...
        <ul class="space-y-4 text-sm text-text-secondary">
          <li v-for="(item, index) in locations" :key="index" class="bg-bg-elevated border border-border rounded-xl p-4">
            <strong class="block text-text-primary mb-1">{{ item.title }}</strong>
            <span class="text-xs text-text-muted leading-5">{{ item.description }}</span>
          </li>
          <li v-if="!locations.length" class="text-text-muted text-sm">暂无数据</li>
        </ul>
      </div>
      ...
      <div class="bg-bg-surface rounded-2xl border border-border p-6">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center text-text-primary font-semibold">
            <svg class="mr-2 text-primary" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
            <span>主要阵营</span>
          </div>
          <button
            v-if="editable"
        ...
        <ul class="space-y-4 text-sm text-text-secondary">
          <li v-for="(item, index) in factions" :key="index" class="bg-bg-elevated border border-border rounded-xl p-4">
            <strong class="block text-text-primary mb-1">{{ item.title }}</strong>
            <span class="text-xs text-text-muted leading-5">{{ item.description }}</span>
          </li>
          <li v-if="!factions.length" class="text-text-muted text-sm">暂无数据</li>
        </ul>
      </div>
    </div>
  </div>
