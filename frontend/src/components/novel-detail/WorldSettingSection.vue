  ...
  <div class="space-y-5">
    ...
    <div class="rounded-2xl border border-[#2A2A2A] bg-[#141414] p-6">
      <div class="flex items-start justify-between gap-4 mb-4">
        <h3 class="text-base font-semibold text-white">核心规则</h3>
        <button
          v-if="editable"
          type="button"
      ...
      <p class="text-[#aaa] leading-7 whitespace-pre-line text-sm">{{ worldSetting.core_rules || '暂无' }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
      ...
      <div class="rounded-2xl border border-[#2A2A2A] bg-[#141414] p-6">
        ...
        <ul class="space-y-3 text-sm text-[#aaa]">
          <li v-for="(item, index) in locations" :key="index" class="bg-[#1C1C1C] border border-[#2A2A2A] rounded-xl p-4">
            <strong class="block text-white mb-1">{{ item.title }}</strong>
            <span class="text-xs text-[#888] leading-5">{{ item.description }}</span>
          </li>
          <li v-if="!locations.length" class="text-[#555] text-sm">暂无数据</li>
        </ul>
      </div>
      ...
      <div class="rounded-2xl border border-[#2A2A2A] bg-[#141414] p-6">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center text-white font-semibold gap-2">
            <svg class="text-[#FFE500]" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
            <span class="text-sm">主要阵营</span>
          </div>
          <button
            v-if="editable"
            type="button"
        ...
        <ul class="space-y-3 text-sm text-[#aaa]">
          <li v-for="(item, index) in factions" :key="index" class="bg-[#1C1C1C] border border-[#2A2A2A] rounded-xl p-4">
            <strong class="block text-white mb-1">{{ item.title }}</strong>
            <span class="text-xs text-[#888] leading-5">{{ item.description }}</span>
          </li>
          <li v-if="!factions.length" class="text-[#555] text-sm">暂无数据</li>
        </ul>
      </div>
    </div>
  </div>
