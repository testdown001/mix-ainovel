...
<template>
  <div class="space-y-4 max-h-96 overflow-y-auto p-1">
    <div v-for="(location, index) in localLocations" :key="index" class="p-4 border border-border rounded-lg bg-bg-elevated relative">
      ...
      <div class="mb-2">
        <label class="block text-sm font-medium text-text-secondary mb-1">地点名称</label>
        <input 
          type="text" 
    ...
    <button @click="addLocation" class="w-full mt-4 px-4 py-2 text-sm font-medium text-primary bg-primary-muted border border-primary/20 rounded-md hover:bg-[rgba(255,255,255,0.05)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary/10">
      + 添加新地点
    </button>
  </div>
</template>
