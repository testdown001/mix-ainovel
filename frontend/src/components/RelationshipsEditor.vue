  ...
  <div class="space-y-4 max-h-[600px] overflow-y-auto p-1">
    ...
    <div v-if="projectId" class="flex items-center justify-between p-3 bg-primary-muted rounded-lg border border-blue-200">
      <div class="flex items-center gap-2">
        ...
        <span class="text-xs text-text-muted">从已生成章节中提取人物关系</span>
      </div>
      <button
    ...
    <!-- 同步状态提示 -->
    <div v-if="syncMessage" class="p-3 rounded-lg text-sm" :class="syncMessageType === 'success' ? 'bg-success-muted text-success border border-green-200' : syncMessageType === 'error' ? 'bg-error-muted text-error border border-error/20' : 'bg-primary-muted text-primary border border-blue-200'">
      {{ syncMessage }}
    </div>

    <div v-for="(relationship, index) in localRelationships" :key="index" class="p-4 border border-border rounded-lg bg-bg-elevated relative">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
        <div>
          <label class="block text-sm font-medium text-text-secondary mb-1">从</label>
          <input type="text" v-model="relationship.character_from" class="w-full p-1 border-b-2 border-border focus:border-border-focus outline-none transition bg-transparent" placeholder="例如：林远" />
        </div>
        <div>
          <label class="block text-sm font-medium text-text-secondary mb-1">到</label>
          <input type="text" v-model="relationship.character_to" class="w-full p-1 border-b-2 border-border focus:border-border-focus outline-none transition bg-transparent" placeholder="例如：苏晴" />
        </div>
      </div>
      <div>
        <label class="block text-sm font-medium text-text-secondary mb-1">关系描述</label>
        <textarea
    ...
    <button @click="addRelationship" class="w-full mt-4 px-4 py-2 text-sm font-medium text-primary bg-primary-muted border border-indigo-200 rounded-md hover:bg-primary-muted focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary/10">
      + 添加新关系
    </button>
  </div>
</template>
