...
当前产品与工程迭代计划见 [ROADMAP.md](ROADMAP.md)。
...
| 模式 | 说明 | 特点 | 解锁档位 |
|------|------|------|------|
| `fast` | 极速模式 | 单版本生成 + 规则人性化，速度优先 | 免费 |
| `standard` | 标准模式 | 多版本生成 + AI 择优 + 六维评审与精修 | 创作者+ |
| `premium` | 旗舰模式 | 标准全流程 + 更多增强开关（强制单版本） | 旗舰 |

> 仅 `fast` / `standard` / `premium` 为有效预设；历史别名（`basic`/`enhanced` → `standard`，`ultimate`/`platinum`/`literary` → `premium`，未知名 → `fast`）会在入口处自动归一化。
