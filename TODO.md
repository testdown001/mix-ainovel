...
① 评估基线 ✅（`1a9d980`，已跑三轮，结论见上）
② 卷级复盘正式重规划 + 卷级发散卡片 ✅（`39c5a20` `9d71757` `ac5a8a0` `428098c`）
③ 两遍制草稿-改写 ✅（`57724f4`）

核心思想缺陷**四条已全部处理**：
~~开环规划~~（②卷级复盘闭环）/ ~~防错非求好~~ ~~约束堆叠上限~~（③拆开两遍）/
~~事实非意义~~（人物意义层 `27df461`）。

### 人物意义层（`27df461`，已上线并**已开启**）

- 抽取四样会改变后续行为的东西：信念变化 / 代价 / 关系质变 / 未言明
- ⚠️ **护栏才是这个特性成立的前提**：不加约束地注入「意义」，模型会把
  「他不再相信示好是无偿的」当台词或旁白**原句写出来**——AI 小说最典型的「说破」，
  比不注入更糟。读侧强制「只通过选择/反应/迟疑/回避体现，严禁直接陈述」，
  段落标题本身也点明「不得直接写出」。已加测试锁死。
- 存储复用 `CharacterState.extra["significance"]`，不新建表
- 成本低（每章一次异步抽取，不阻塞正文）

### ③ 的交付说明

- 病灶不是"只生成了一遍"，而是**所有约束都堆进第一遍**：26 个段落里约一半是
  「不许犯什么错」，模型注意力被清单吃掉
- 第一遍只给事实与方向（求好），第二遍拿草稿 + 全部规则做**改写**（防错）
- 切分用**黑名单**：新增段落默认进草稿 → 退化回"一次性堆叠"这个已知状态，
  而非悄悄饿掉草稿的关键事实
- 第二遍只对**选中的最佳稿**跑；缩水超四成/校验不过/调用失败一律退回草稿
- 🔜 **默认关且建议先算账**：`TWO_PASS_DRAFT_ENABLED` 开启后每章多一次**整章级**
  LLM 调用，flagship 单章成本近乎翻倍。prod compose 已透传，开关一改即生效。
  **建议开启前先小样本对比质量增益是否值这个钱**（这正是评估基线该派上用场的地方）。
...
## 🎚️ 质量回路开关（后台「✨质量回路」面板，`e02c919`）

四项均为 **flagship 独占、代码默认关**，配置在 SystemConfig `quality_loop.*`
（env `*_ENABLED` 只是首次启动种子），**改完下一次生成即生效、无需重启**。
...
| 卷级复盘重规划 | 低 · 异步 | 测试阶段**开** |
| 人物意义层 | 低 · 异步 | 测试阶段**开** |
| 两遍制草稿-改写 | **高 · 生成链路内，单章成本近乎翻倍** | 测试阶段**开**（观察后决定是否保留） |

⚠️ 服务器 `deploy/.env` 里四个 env 已全部置回 false（默认态），实际开关只看后台面板。
🔜 **观察重点**：单章耗时变化、生成失败率、以及正文里有没有出现「把意义直接说破」的句子
（人物意义层最大的风险；护栏已加但需实测确认）。
